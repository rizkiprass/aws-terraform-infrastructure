locals {
  ...
  cwatch-alarm-instanceid-linux = [
    module.ec2_bastion.instance_id, module.ec2_app1.instance_id] #TODO: Update with new instance for linux
}
resource "aws_sns_topic" "alert-sns-topic" {
  name = format("%s-%s-resource-alert-topic", var.customer, var.project)
